...
port_alterid_set(){
    ...
    [[ -z ${port} ]] && port="443"
    stty erase '^H' && read -p "请输入alterID（default:88）:" alterID
    [[ -z ${alterID} ]] && alterID="88"
}
modify_port_UUID(){
...
v2ray_conf_add(){
    cd /etc/v2ray
    wget https://raw.githubusercontent.com/eagleandsanake/first/eagleandsanake-patch-1/config.json
modify_port_UUID
judge "V2ray 配置修改"
}
nginx_conf_add(){

...
time_modify(){
    ...
    time_now="date -R"
    echo -e "当下时间：${GreenBG} $time_now ${Font}"
}
dependency_install(){
...
web_camouflage(){
    ...
    git clone https://github.com/eagleandsanake/first.git
    judge "web 站点伪装"   
}
v2ray_install(){
...
domain_check(){
    stty erase '^H' && read -p "请输入你的域名信息(eg:exmple.com):" domain
...
v2ray_conf_add(){
    cd /etc/v2ray
    wget https://raw.githubusercontent.com/eagleandsanake/first/eagleandsanake-patch-1/config.json -O config.json
modify_port_UUID
judge "V2ray 配置修改"
}
nginx_conf_add(){
...
bbr_auto_open(){
    ...
    echo -e "当下时间：${GreenBG} $bbrinfo ${Font}"
}
show_information(){
    clear

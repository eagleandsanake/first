...
path_config() {
	...
	while :; do
		...
		*[/$]*)
			echo
			echo -e "分流的路径不能包含$red / $none或$red $ $none这两个符号.... "
			echo
			error
			;;
		*)
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			echo
			echo -e "$red 安装失败了$none"
			echo
			echo -e " 请确保你有完整安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
...
*)
	echo
	echo -e " 你输入的这个参数 <$red $args $none> 错误"
	echo
